...
import { queueElement, fileDatabase } from "../types"
import { httpRequest } from "../utilities/helper"
import { v4 as uuidv4 } from "uuid";

//IMPLEMENT ASYNCHRONOUS FUNCTIONS HERE

export async function input_handler(): Promise<void>{
	/**
	 * NOTES:
	...
	const inputBuffer = new fileDatabase("./fileDatabases/inputBufferDB")
	const approvalList = new fileDatabase("./fileDatabases/approvalListDB")

	input_fromSources(approvalList)
	input_fromBuffer(approvalList,inputBuffer)
}

export async function input_fromBuffer(approvalList: fileDatabase, inputBuffer: fileDatabase): Promise<void>{
...
async function input_fromSources(approvalList: fileDatabase): Promise<void>{
	/**
...
function preProcess_input(input: queueElement): queueElement{
	/**
...
async function input_customerCare(): Promise<Array<queueElement>>{
	/**
...
async function input_testingTime_unsubscriptions(): Promise<Array<queueElement>>{
	/**
